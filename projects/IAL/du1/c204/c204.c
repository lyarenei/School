...
void doOperation(tStack *s, char c, char *postExpr, unsigned *postLen)
{
    ...
    case '*':
    case '/':
        while (stackFull(s) == 0 || stackEmpty(s) != 0)
        {
            if (stackEmpty(s) == 0)
            ...
            {
                if ((stackTopChar != '+' && stackTopChar != '-') && (stackTopChar != '*' && stackTopChar != '/'))
                {
                    if (stackFull(s) == 0)
                        stackPush(s, c);
                    break;
...
char *infix2postfix(const char *infExpr)
{
    ...
    stackInit(&stack);
    unsigned postPos = 0;
    char *postExpr = calloc(MAX_LEN, sizeof(char));

    if (postExpr == NULL)
        return NULL;

    for (unsigned pos = 0; infExpr[pos] != '\0'; pos++)
        doOperation(&stack, infExpr[pos], postExpr, &postPos);
